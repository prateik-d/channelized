...
<div class="event-search-result">
    <div class="event-search-result-header">
        ...
        <div class="event-search-result-action">
            <a href="{{ route(Auth::user()->roles[0]->name.'.events') }}" class="btn btn-primary btn-shadow">Search events</a>
            <a href="{{ route(Auth::user()->roles[0]->name.'.events.calender') }}" class="btn btn-light btn-shadow">My Calendar</a>
        </div>
    </div>
    @php
        $search_parameter = [];
        ...
        if(isset($partner_filter->type_of_event_id) && $partner_filter->type_of_offer_id != 0){
            $search_parameter['type_of_event_id'] =$type_of_events->where('id',$partner_filter->type_of_event_id)->first()->name;
        }
    @endphp
    ...
    <div class="event-search-result-container">
        ...
        <div class="event-search-filter signUpform">
            ...
            @endif
            <form method="POST" action="{{ route(Auth::user()->roles[0]->name.'.events.store') }}">
                @csrf
                <div class="filterSroll eventdropdown">
                    ...
                    <div class="form-group filter-row">
                        <select id="typeOfEventFilter" class="filterevent btn-shadow" name="type_of_event" >
                            <option value="0" selected>Type of event</option>
                            @foreach($type_of_events as $single)
                                <option value="{{$single->id}}" {{ (isset($partner_filter->type_of_event_id) && $partner_filter->type_of_event_id==$single->id?'selected':'') }}>{{$single->name}}</option>
                            @endforeach
                        </select>
                        <div class="inputInnerShadow"></div>
                    </div> 
  ...
  <button class="btn btn-primary btn-list grid-btn" check="0"><i class="fa fa-list" aria-hidden="true"></i></button>
            ...
            <div class="card-list event-crd scrollbar" id="scrollbar-custombox">
...
//            alert(deltlink); return false;
        ...
        $(document).on('click', '#reset_btn', function(){
            ...
            var filter_arry = {};
            var filterlink = "{{route(Auth::user()->roles[0]->name.'.event.filter')}}";
            var token = $("meta[name='csrf-token']").attr("content");
            startdt=null;
            enddt=null;
        ...
        function filter_event(){
            ...
            if($('#typeOfEventFilter').val()!=null){
                filter_arry['type_of_event'] = $('#typeOfEventFilter').val(); 
            }
            if(startdt!=null){
