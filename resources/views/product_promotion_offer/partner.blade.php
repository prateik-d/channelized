...
<div class="event-search-result">
    <div class="event-search-result-header">
        ...
        <div class="event-search-result-action">
            <a href="{{ route(Auth::user()->roles[0]->name.'.product-promotions-offers') }}" class="btn btn-primary btn-shadow">Search Product Promotions & Offers</a>
            <a href="{{ route(Auth::user()->roles[0]->name.'.product-promotions-offers.calender') }}" class="btn btn-light btn-shadow">My Calendar</a>
        </div>
    </div>
    @php
        $search_parameter = [];
        ...
        if(isset($partner_filter->type_of_offer_id) && $partner_filter->type_of_offer_id != 0){
            $search_parameter['type_of_offer_id'] = $type_of_offers->where('id',$partner_filter->type_of_offer_id)->first()->name;
        }
    @endphp
    ...
    <div class="event-search-result-container">
        ...
        <div class="event-search-filter signUpform">
            ...
            @endif
            <form method="POST" action="{{ route(Auth::user()->roles[0]->name.'.product_promotions_offers.store') }}">
                @csrf
                <div class="filterSroll eventdropdown">
                    ...
                    <div class="form-group filter-row">
                        <select id="typeOfOfferFilter" class="filterevent btn-shadow" name="type_of_offer" >
                            <option value="0" selected>Offers Type</option>
                            @foreach($type_of_offers as $single)
                                <option value="{{$single->id}}" {{ (isset($partner_filter->type_of_offer_id) && $partner_filter->type_of_offer_id==$single->id?'selected':'') }}>{{$single->name}}</option>
                            @endforeach
                        </select>
                        <div class="inputInnerShadow"></div>
                    </div> 
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                             @if($event->banner)
                                ...
                                <!-- <div class="card-bgimg" style="background:url('{{ asset('public/uploads/events') }}/{{$event->banner}}')">
...
//            alert(deltlink); return false;
        ...
        $(document).on('click', '#reset_btn', function(){
            ...
            var filter_arry = {};
            var filterlink = "{{route(Auth::user()->roles[0]->name.'.incentives.filter')}}";
            var token = $("meta[name='csrf-token']").attr("content");
            startdt=null;
            enddt=null;
        ...
        function filter_event(){
            ...
            if($('#typeOfOfferFilter').val()!=null){
                filter_arry['type_of_offer'] = $('#typeOfOfferFilter').val(); 
            }
            if(startdt!=null){
            ...
            var filterlink =  "{{route(Auth::user()->roles[0]->name.'.offers.filter')}}";
            var token = $("meta[name='csrf-token']").attr("content");
            /* debugger; */
            var sum = 0;
