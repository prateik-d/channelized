...
<div class="admin-container">
    ...
    @endif
    <form method="POST" id="event_update" class="event-add-form" action="{{ route(Auth::user()->roles[0]->name.'.rewards-incentives.update',Crypt::encrypt($event->id)) }}"  enctype='multipart/form-data'>
                        ...
                        <div class="create-event-form-input">
                            <div class="create-event-form-input">
                                <select class="event-dropcreate @error('type') is-invalid @enderror" id="type" name="type" required="">
                                    <option value="0" {{(@old('type',$event->type_of_incentive_id)== NULL ? 'selected':'')}}>Select</option>
                                    @foreach($type_of_events as $single)
                                        <option value="{{$single->id}}" {{(@old('type',$event->type_of_incentive_id)==$single->id ? 'selected':'')}}>{{$single->name}}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                    <span class="invalid-feedback" role="alert">
                ...
                <div class="create-event-form">
                    <div class="create-event-form-row">
                     ...
                     <div class="create-event-form-row">
                        ...
                        <div class="create-event-form-input">
                            <textarea rows="5" class="@error('short_summary') is-invalid @enderror" id="short_summary" name="short_summary" placeholder="Short Description of your Rewards & Incentives (not more than 150 words)" required>{{old('short_summary',$event->short_summary)}}</textarea>
                            @error('short_summary')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
