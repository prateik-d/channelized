...
<div class="event-search-result">
    <div class="event-search-result-header">
        ...
        <div class="event-search-result-action">
            <a href="{{ route(Auth::user()->roles[0]->name.'.rewards-incentives') }}" class="btn btn-primary btn-shadow">Search Rewards & Incentives</a>
            <a href="{{ route(Auth::user()->roles[0]->name.'.rewards-incentives.calender') }}" class="btn btn-light btn-shadow">My Calendar</a>
        </div>
    </div>
    @php
        $search_parameter = [];
        ...
        if(isset($partner_filter->type_of_incentive_id) && $partner_filter->type_of_offer_id != 0){
            $search_parameter['type_of_incentive_id'] =$type_of_incentives->where('id',$partner_filter->type_of_incentive_id)->first()->name;
        }
    @endphp
    ...
    <div class="event-search-result-container">
        ...
        <div class="event-search-filter signUpform">
            ...
            @endif
            <form method="POST" action="{{ route(Auth::user()->roles[0]->name.'.rewards_incentives.store') }}">
                @csrf
                <div class="filterSroll eventdropdown">
                    ...
                    <div class="form-group filter-row">
                        <select id="typeOfIncentiveFilter" class="filterevent btn-shadow" name="type_of_incentive" >
                            <option value="0" selected>Incentive Type</option>
                            @foreach($type_of_incentives as $single)
                                <option value="{{$single->id}}" {{ (isset($partner_filter->type_of_incentive_id) && $partner_filter->type_of_incentive_id==$single->id?'selected':'') }}>{{$single->name}}</option>
                            @endforeach
                        </select>
                        <div class="inputInnerShadow"></div>
                    </div> 
...
//            alert(deltlink); return false;
        ...
        function filter_event(){
            ...
            if($('#typeOfIncentiveFilter').val()!=null){
                filter_arry['type_of_incentive'] = $('#typeOfIncentiveFilter').val(); 
            }
            if(startdt!=null){
