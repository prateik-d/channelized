...
<div class="admin-container">
    ...
    @endif
    <form method="POST" id="event_update" class="event-add-form" action="{{ route(Auth::user()->roles[0]->name.'.technical-education.update',Crypt::encrypt($event->id)) }}"  enctype='multipart/form-data'>
        ...
        <div class="col-md-8">
            <div class="create-form">
                <div class="create-event-form">
                    ...
                    <div class="create-event-form-row">
                        <div class="create-event-form-label">Event type</div>
                        <div class="create-event-form-input">
                            <div class="create-event-form-input">
                                <select class="event-dropcreate @error('type') is-invalid @enderror" id="type" name="type" required="">
                                    <option value="0" {{(@old('type',$event->type_of_technical_education_id)==NULL ? 'selected':'')}}>Select</option>
                                    @foreach($type_of_events as $single)
                                        <option value="{{$single->id}}" {{(@old('type',$event->type_of_technical_education_id)==$single->id ? 'selected':'')}}>{{$single->name}}</option>
                                    @endforeach
                                </select>
                                @error('type')
                                    <span class="invalid-feedback" role="alert">
                            ...
                            @if(old('agdst') || old('agdet') || old('topic') || old('spkrnm'))
                                @for( $i=0; $i < count(old('agdst')); $i++)
                                    <div class="create-event-form-input {{($i==0 ? '' : 'mt-2')}}">
                                        <input type="hidden" name="eagnd_id[]" value="{{ old('eagnd_id.'.$i) }}" />
                                ...
                                @endfor
                            @elseif($event->eventagendas()->count() > 0)
                                @foreach($event->eventagendas()->get() as $agenda)
                                    <div class="create-event-form-input {{($loop->first ? '' : 'mt-2')}}">
                                        <input type="hidden" name="eagnd_id[]" value="{{$agenda->id}}" />
                                        <input id="agdst" type="text" class="timepicker input-120" name="agdst[]" value="{{date('h:i A', strtotime($agenda->start))}}" autocomplete="agdst" placeholder="hh:mm a">
                                        <div class="input-18 mr-1 ml-1">-</div>
                                        <input id="agdet" type="text" class="timepicker input-120" name="agdet[]" value="{{date('h:i A', strtotime($agenda->end))}}" autocomplete="agdet" placeholder="hh:mm a">
                                        <div class="input-18 mr-1 ml-1">-</div>
                                        <input id="topic" type="text" class="input-240" name="topic[]" value="{{$agenda->topic}}" autocomplete="topic" placeholder="Topic">
                                        <div class="input-18 mr-1 ml-1">-</div>
                                        <input id="spkrnm" type="text" class="input-240" name="spkrnm[]" value="{{$agenda->speaker_name}}" autocomplete="spkrnm" placeholder="Speaker's name">
                                        <div class="input-50 mr-1 ml-1 plusevent-btn">
                                            @if($loop->first)
                                                <a href="javascript:void(0)" class="add_agenda">+</a>
                                            @else
            ...
            <div class="create-form">
                <div class="create-event-form">
                    ...
                    <div class="create-event-form-row">
                        ...
                        <div class="create-event-form-input">
                            <textarea rows="10" class="@error('summary') is-invalid @enderror" id="short_summary" name="short_summary" placeholder="Short description of your event (not more than 350 words)" required>{{old('short_summary',$event->short_summary )}}</textarea>
                            @error('short_summary')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
